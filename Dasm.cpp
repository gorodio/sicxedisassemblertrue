...
void Dasm::loadDataStorage() {
    ...
    int i = 2;
    for (i = 2; i < symStorage.size()-1; i++) {    //fill symbol storage
    ...
    for (int j = i; j<symStorage.size()-1; j++) {  //fill literal storage
...
// reads an object file and sym file and make sic file
void Dasm::openFiles(char *objFile) {
    ...
    symFile.append(".sym");                         //remove .obj extension and insert .sym
    symStream.open(symFile.c_str());
    if (!symStream.is_open()){
        cout << "'.sym' file not found." << endl;
        exit(EXIT_FAILURE);
    }
    string sicFile = objFile;				//insert .sic extension
...
int Dasm::analyzeFormat3(OpCode code, int opCode, int row, int current) {
    string opName = code.getOpName(opCode);                // find instruction
    bool nixbpe[6];
...
// analyzes each portion of text record object code
